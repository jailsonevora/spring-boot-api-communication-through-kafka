...
import com.BusinessEntityManagementSystem.dataAccessObject.IAddressRepository;
...
public class AddressController implements IGenericCRUD<AddressModel> {
    ...
    public ResponseEntity<Void> create(@Valid @RequestBody AddressModel newAddress){
        ...
        responseHeaders.setLocation(ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(newAddress.getId()).toUri());
    ...
    @Transactional
    @RequestMapping(value = "/addressos/{id}", method = RequestMethod.PUT, produces = "application/json")
