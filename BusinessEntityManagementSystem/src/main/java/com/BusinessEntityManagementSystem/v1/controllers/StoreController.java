...
import com.BusinessEntityManagementSystem.dataAccessObject.IStoreRepository;
...
public class StoreController implements IGenericCRUD<StoreModel> {
    ...
    public ResponseEntity<Void> create(@Valid @RequestBody StoreModel newStore){
        ...
        responseHeaders.setLocation(ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(newStore.getId()).toUri());
