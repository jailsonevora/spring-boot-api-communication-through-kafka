...
import com.BusinessStatisticsUnitFiles.dataAccessObject.IPartnerRepository;
...
public class PartnerController implements IGenericCRUD<PartnerModel> {
    ...
    public ResponseEntity<Void> create(@Valid @RequestBody PartnerModel newPartner){
        ...
        responseHeaders.setLocation(ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(newPartner.getPartnerId()).toUri());
